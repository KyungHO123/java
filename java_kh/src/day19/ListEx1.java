...
public class ListEx1 {
	...
	public static void main(String[] args) {
		/*제네릭 인터페스인 List의 타입을 지정하지 않은 경우
		 *  - 타입에 최상위 클래스인 Object가 자동으로 들어 
		 ...
		 *    중 아무나 올 수 있다.
		 *    - list3 = new ArrayLisr<String>();= > O
		 *    - list3 = new LinkedList<String>();= > O
		 *    
		 *  - list4는 ArrayList 클래스를 이용한 객체만 올 수 있다.  
		 *    - list3 = new ArrayLisr<String>(); = > O
		 *    - list3 = new LinkedList<String>();= > X
		 * */
		List<String> list3;
		ArrayList<String> list4;
		
		/* add(객체) : Collection에서 제공, 객체를 추가하고 추가 여부를 알려줌 
		 *  - 리스트에서는 무조건 true 
		...
		/* contains (객체) : Collection에서 제공, 컬렉션에 객체가 있는지 없는지를 알려줌
		...
		/* indexOf(객체) : List에서 제공, 리스트에 객체가 몇번지에 있는지 알려줌. 없으면 -1
		 *  - 동작 방식은 contains와 같음
		 * */
		System.out.println(list5.indexOf("abc"));
		
		/* remove(번지) : List에서 제공, 번지에 있는 객체를 제거한 후 제거한 객체를 반환
		 * remove(객체) : Collection에서 제공, 객체와 일치하는 요소를 제거 후 제거 여부를 반환
		 * */
		list5.remove(0);
		list5.remove("def");
		...
		list5.add("123");
		list5.add("456");
		/* iterator() : Collection에서제공, 컬렉션을 이용하여 반복자를 생성해서 리턴 
		 * 				=> 반복자를 이용하여 반복문을 활용하기 위해
		 * 
		 * */
		...
		/* size() : Collection에서 제공 , 컬렌션의 크기를 반환
		 * */
		System.out.println(list.size());
		/* forEach() : Collection에서 제공 , 람다식을 이용하여 컬렉션에서 하나 꺼낸 값을 활용할 때 사용 
		 * */
		list5.forEach(s->System.out.println(s));
		
		/* sort() : List에서만 제공,람다식을 이용해서 정렬 기준을 정해주면 정해진 정렬기준대로 리스트를 정렬
